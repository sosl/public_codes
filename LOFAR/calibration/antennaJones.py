...
def getHamakerJones(obsTimes,stnPos,stnRot,srcDirection,freq,
   ...
   for ti in range(len(obsTimesArr)):
       ...
       theta=el
       (JXphi,JXtheta,JYphi,JYtheta)=HamakerModel.getJonesByAzEl_hba(freq,phi,theta)
...
#         print quantity(obsTimes.get_value()[ti],obsTimes.get_unit()).formatted("YMD"),Jn[ti,0,0].real,Jn[ti,0,0].imag,Jn[ti,0,1].real,Jn[ti,0,1].imag,Jn[ti,1,0].real,Jn[ti,1,0].imag,Jn[ti,1,1].real,Jn[ti,1,1].imag
...
if __name__ == "__main__":
   ...
   opt.add_option('-n','--negDec', dest='negDec', default=False, action='store_true', help='Use this for negative decliations')
   ...
   if options.obsID != "null" :
      ...
      for stnName in stnNames:
          args[0]=stnName
          printJones(args,model)
   elif len(args) != 7:
      opt.error("incorrect number of arguments, got " + str(len(args)))
   else :
      printJones(args,model)
