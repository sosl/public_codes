...
parser.add_argument('INPUT_ARCHIVE', nargs='+', help="Archives for which the bandpass should be corrected")
...
for ar_name in args.INPUT_ARCHIVE:
    ...
    if args.plot:
        # Obtain the baseline statistics which we use to derive the bandpass for this observation
        ...
        for ipol in range(ar.get_npol()):
            for ichan in range(ar.get_nchan()):
                if ichan in non_zeroes[0]:
                    ...
                    prof.set_weight(0.0)
    if args.verbose:
        print "Unloading the bandpass-corrected archive to " + outname +args.ext
